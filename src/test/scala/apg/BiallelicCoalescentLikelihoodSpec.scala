    ...
    "pi = (0.5, 0.5)" when {
      ...
      "intervals = [(length = Inf, m = 2, k = 2, u = 1, v = 1, 1/Ne = 1)]" when {
        ...
        "samples = [[1, 0, 0]]" should {
          "be 0.3" in {
            assert(0.3 === new BiallelicSiteLikelihood(0.5, new Base(BiallelicCoalescentInterval(length = Double.PositiveInfinity, m = 2, k = 2, u = 1, v = 1, coalRate = 1, coalIndex = 0, IndexedSeq(1.0, 0.0, 0.0).apply))).evaluate)
          }
        }

        "samples = [[0, 1, 0]]" should {
          "be 0.4" in {
            assert(0.4 === new BiallelicSiteLikelihood(0.5, new Base(BiallelicCoalescentInterval(length = Double.PositiveInfinity, m = 2, k = 2, u = 1, v = 1, coalRate = 1, coalIndex = 0, IndexedSeq(0.0, 1.0, 0.0).apply))).evaluate)
          }
        }

        "samples = [[0, 0, 1]]" should {
          "be 0.3" in {
            assert(0.3 === new BiallelicSiteLikelihood(0.5, new Base(BiallelicCoalescentInterval(length = Double.PositiveInfinity, m = 2, k = 2, u = 1, v = 1, coalRate = 1, coalIndex = 0, IndexedSeq(0.0, 0.0, 1.0).apply))).evaluate)
          }
        }
