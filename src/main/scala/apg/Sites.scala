...
object Sites {
  ...
  def apply[D[X], B[X]](timePoints: Seq[Array[TimePoint]], threshold: Double = 0)(implicit distributed: Distributed[D, B]): D[LinearSeq[TimePoint]] = {
    ...
    distributed.range(0, timePoints.head.length).map(i => distributed.retrieve(broadcastedTimePoints).map(_(i)).sorted.to[LinearSeq]).filter(_.forall(_.redCountPartial.exists(_ > threshold)))
  }
