...
class PoissonBinomialDistribution(val p: Seq[Double]) extends (Int => Double) {
  ...
  private[this] val mass = if (p.isEmpty)
    Vector(1.0)
  else {
    val alpha = pp.product
    val s = pp.map(pk => -(1 - pk) / pk)
    val r = Traversable.tabulate(Np + 1) { n =>
    ...
    val fft = new DoubleFFT_1D(Np + 1)
    fft.complexForward(r)
    r.grouped(2).map(_(0) * alpha / (Np + 1)).toVector
  }
